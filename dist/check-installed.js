...
function iterSkills(root = repoRoot()) {
  const skills = walkSkillFiles(join(root, "skills")).map((skillMd) => parseSkillInfo(skillMd, { sourceOnly: false, label: "skills" }));
  const repoLocalSkills = walkSkillFiles(join(root, ".agents", "skills")).map((skillMd) => parseSkillInfo(skillMd, { sourceOnly: true, label: "repo-local" }));
...
function compareSkill(sourceDir, installedDir) {
  ...
  for (const relpath of [...sourceFiles].filter((path) => installedFiles.has(path)).sort()) {
    if (source.get(relpath) !== installed.get(relpath))
      problems.push(`out of date: ${join(installedDir, relpath)}`);
  }
  return problems;
}
...
var listNewInstalls = args.has("--new-installs");
var skills = iterSkills();
var installable = installableSkills(skills);
var sourceOnly = sourceOnlySkills(skills);
if (skills.length === 0) {
...
if (listNewInstalls) {
  for (const skill of installable) {
    ...
    if (roots.some((root) => !existsSync2(join2(root, skill.name)))) {
...
for (const skill of installable) {
  ...
  for (const root of roots) {
    const problems = compareSkill(skill.directory, join2(root, skill.name));
...
var installedRoots = [join2(homedir2(), ".agents", "skills"), join2(homedir2(), ".claude", "skills")];
for (const skill of sourceOnly) {
  for (const root of installedRoots) {
    const installedDir = join2(root, skill.name);
...
var universalCount = installable.filter((skill) => skill.targets.includes("universal")).length;
var claudeCount = installable.filter((skill) => skill.targets.includes("claude-code") && !skill.targets.includes("universal")).length;
console.log(`OK: ${universalCount} universal skill(s), ${claudeCount} Claude-only skill(s), and ${sourceOnly.length} source-only skill(s) are in sync.`);
