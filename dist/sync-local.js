...
function script(name) {
  const tsPath = join2(import.meta.dir, name);
  if (existsSync2(tsPath))
    return tsPath;
  return join2(import.meta.dir, name.replace(/\.ts$/, ".js"));
}
async function runInherited(command) {
...
async function confirmNewInstalls(newInstalls) {
  if (!newInstalls)
    return;
  console.log();
...
`).filter(Boolean))
  ...
  if (!process.stdin.isTTY) {
    ...
    console.error("Re-run interactively, or set SKILLS_SYNC_CONFIRM_NEW=1 if you intentionally want to install them.");
    process.exit(1);
  }
  const rl = createInterface({ input: stdin, output: stdout });
  const reply = (await rl.question("Install these new skills? [y/N] ")).trim();
  rl.close();
  if (["y", "Y", "yes", "YES"].includes(reply))
    return;
  console.log("Cancelled; no new skills installed.");
  process.exit(1);
}
console.log("Checking installed skills...");
var checkCode = await runInherited(["bun", script("check-installed.ts")]);
if (checkCode === 0) {
  console.log("Skills are already in sync.");
  process.exit(0);
}
var newInstalls = await output(["bun", script("check-installed.ts"), "--new-installs"]);
