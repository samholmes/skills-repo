...
var sourceOnlyNames = [...new Set(sourceOnlySkills(iterSkills()).map((skill) => skill.name))].sort();
var installedSourceOnlyNames = sourceOnlyNames.filter((name) => installedRoots.some((root) => existsSync2(join2(root, name))));
...
for (const name of installedSourceOnlyNames)
  console.log(`  - ${name}`);
process.chdir(homedir2());
