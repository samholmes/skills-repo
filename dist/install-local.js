...
import { existsSync as existsSync2, lstatSync, readdirSync, readFileSync, readlinkSync } from "fs";
import { homedir as homedir2 } from "os";
import { join as join2, relative } from "path";
var ignoredNames = new Set([".DS_Store", "__pycache__"]);
var ignoredSuffixes = [".pyc", ".swp", ".tmp"];
function repoRoot() {
  return process.cwd();
}
function frontmatterFromMarkdown(path) {
...
function targetsFrom(frontmatter, sourceOnly) {
  if (sourceOnly)
    return [];
  const metadata = metadataFrom(frontmatter);
  const rawTargets = metadata.targets ?? metadata.target;
  if (Array.isArray(rawTargets)) {
    return rawTargets.map(stringValue).filter((target2) => Boolean(target2));
  }
...
function parseSkillInfo(skillMd, options) {
...
function walkSkillFiles(container) {
  if (!existsSync2(container))
    return [];
  const found = [];
...
function iterSkills(root = repoRoot()) {
  const skills = walkSkillFiles(join2(root, "skills")).map((skillMd) => parseSkillInfo(skillMd, { sourceOnly: false, label: "skills" }));
  const repoLocalSkills = walkSkillFiles(join2(root, ".agents", "skills")).map((skillMd) => parseSkillInfo(skillMd, { sourceOnly: true, label: "repo-local" }));
  return [...skills, ...repoLocalSkills];
}
function installableSkills(skills = iterSkills()) {
  return skills.filter((skill) => !skill.sourceOnly && !skill.internal);
}
function sourceOnlySkills(skills = iterSkills()) {
  return skills.filter((skill) => skill.sourceOnly || skill.internal);
}
function installedRootsForSkill(skill) {
  if (skill.targets.includes("universal"))
    return [join2(homedir2(), ".agents", "skills")];
  if (skill.targets.includes("claude-code"))
    return [join2(homedir2(), ".claude", "skills")];
  return skill.targets.map((target) => installedRootForAgent(target)).filter((root) => Boolean(root));
}
function installedRootForAgent(agent) {
  if (agent === "universal")
    return join2(homedir2(), ".agents", "skills");
...
function walkFiles(directory) {
  ...
  const walk = (current) => {
    for (const entry of readdirSync(current, { withFileTypes: true })) {
      ...
      if (relpath.split("/").some(shouldIgnore))
        continue;
      if (entry.isDirectory()) {
        walk(path);
...
function compareSkill(sourceDir, installedDir) {
  if (!existsSync2(installedDir))
    return [`missing install: ${installedDir}`];
  if (!lstatSync(installedDir).isDirectory())
    return [`installed path is not a directory: ${installedDir}`];
  ...
  for (const relpath of [...sourceFiles].filter((path) => !installedFiles.has(path)).sort()) {
    problems.push(`missing installed file: ${join2(installedDir, relpath)}`);
  }
  for (const relpath of [...installedFiles].filter((path) => !sourceFiles.has(path)).sort()) {
    problems.push(`extra installed file: ${join2(installedDir, relpath)}`);
  }
  for (const relpath of [...sourceFiles].filter((path) => installedFiles.has(path)).sort()) {
    if (source.get(relpath) !== installed.get(relpath))
      problems.push(`out of date: ${join2(installedDir, relpath)}`);
  }
  return problems;
}
...
var discoveredAgents = discoverAgents().map((agent) => agent.slug);
...
var discovered = new Set(discoveredAgents);
var skills = installableSkills(iterSkills());
if (skills.length === 0) {
  console.log("No installable skills found; skipping.");
  process.exit(0);
}
async function installSkillSet(skillNames, agents2) {
  ...
  console.log("Target agents:", agents2.join(" "));
  console.log(`Running: bunx skills add ./skills --global --yes --skill ${skillNames.join(" ")} --agent ${agents2.join(" ")}`);
  await $`bunx skills add ./skills --global --yes --skill ${skillNames} --agent ${agents2}`;
}
var universalSkills = skills.filter((skill) => skill.targets.includes("universal")).map((skill) => skill.name);
await installSkillSet(universalSkills, discoveredAgents);
var targeted = new Map;
